...
class GaussianProcessEnsemble(AbstractModel):
    def __init__(self, config_space, types: np.ndarray,
                 bounds: np.ndarray, s_max, eta, weight_list, fusion_method, rng, **kwargs):
        ...
        for index, item in enumerate(np.logspace(0, self.s_max, self.s_max + 1, base=self.eta)):
            ...
            self.surrogate_container[r] = create_gp_model('gp', config_space, types, bounds, self.rng)
...
class GaussianProcessEnsemble2(AbstractModel):
    def __init__(self, config_space, types: np.ndarray,
                 bounds: np.ndarray, r_list, weight_list, fusion_method, rng, **kwargs):
        super().__init__(types=types, bounds=bounds, **kwargs)

        self.fusion = fusion_method
        self.surrogate_weight = dict()
        self.surrogate_container = dict()
        self.surrogate_r = r_list
        self.weight_list = weight_list
        self.rng = rng
