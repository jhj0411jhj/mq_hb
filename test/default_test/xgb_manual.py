...
python test/default_test/xgb_manual.py --dataset spambase --n_jobs 15 --seed 47
...
config = XGBoost.get_cs().get_default_configuration()   # todo

with timeit('%s-%d' % (dataset, seed)):
    print('=== xgb manual param (my default)===')
    perf, test_perf, evals_result, train_time = objective_func(config, x_train, x_val, x_test, y_train, y_val, y_test)
    print(evals_result)
    ...
    file_name = 'xgb_manual-%s-%04d-%s.pkl' % (dataset, seed, timestamp)
    try:
