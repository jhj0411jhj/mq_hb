...
class async_mqMFES_v37(async_mqHyperband):
    """
    ...
    v37: random (equal weight, except last) + abohb
    """
    ...
    def get_next_n_iteration(self):
        ...
        if self.use_weight_init and len(self.incumbent_configs) >= 3 * 8:  # todo: replace 8 by full observation num
            choose_weights = [1. / self.s_max] * self.s_max + [0.]
            next_n_iteration = self.rng.choice(self.iterate_r, p=choose_weights)
            self.logger.info('random choosing next_n_iteration=%d. choose_weights: %s.'
                             % (next_n_iteration, choose_weights))
