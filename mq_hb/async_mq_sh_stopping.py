...
class async_mqSuccessiveHalving_stopping(async_mqBaseFacade_stopping):
    """
    ...
    def decide_stopping(self, config, perf, n_iteration):
        ...
        updated = False
        job = None
        for _job in self.bracket[rung_id]['jobs']:
            _job_status, _config, _perf, _extra_conf = _job
            if _config == config:
        ...
        if rung_id == len(self.bracket) - 1:  # top rung
            job[0] = STOPPED
            return -1
        ...
        if not exist:
            ...
            if perf < MAXINT and idx < ceil(len(complete_jobs) / self.eta):
                ...
                next_extra_conf = extra_conf.copy()  # todo
