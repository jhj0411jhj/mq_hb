...
python test/nas_benchmarks/benchmark_nasbench201.py --dataset cifar10-valid --mths ams --R 27 --n_workers 4 --runtime_limit 86400 --rep 1 --start_id 0
...
parser.add_argument('--stop_iterations', type=str)
parser.add_argument('--window_size', type=int)
parser.add_argument('--model_iterations', type=str)
...
from test.utils import parse_list_str

stop_iterations = parse_list_str(args.stop_iterations) if args.stop_iterations is not None else None
window_size = args.window_size
model_iterations = parse_list_str(args.model_iterations) if args.model_iterations is not None else None
        ...
        with timeit('%s all' % dataset):
            for algo_name in mths:
                with timeit('%s %d %d' % (algo_name, start_id, rep)):
                    ...
                    assert algo_class in stopping_mths, 'This benchmark file is designed for stopping methods only!'
                    ...
                    for i in range(start_id, start_id + rep):
                        ...
                        method_str = algo_name
                        if R != 27:
                            method_str += '-%d' % R
                        if stop_iterations is not None:
                            method_str += '-s%d' % len(stop_iterations)
                        if model_iterations is not None:
                            method_str += '-m%d' % len(model_iterations)
                        if window_size is not None:
                            method_str += '-w%d' % window_size
                        method_str += '-n%d' % n_workers
