...
class async_mqMFES_v4(async_mqHyperband):
    """
    The implementation of Asynchronous MFES (combine ASHA and MFES)
    before fix
    no median
    promotion start threshold, fix start stage
    """
    ...
    def __init__(self, objective_func,
        ...
        # fix start stage
        self.all_promotion_started = False
    ...
    def get_job(self):
        ...
        if next_n_iteration == self.R:
            # number of configs in every rung is not less than promotion_start_threshold
            self.all_promotion_started = True
    ...
    def get_next_n_iteration(self):
        ...
        # fix start stage:
        # do not sample new config into higher rungs before number of configs
        # in every rung is not less than promotion_start_threshold
        if next_n_iteration > 1.0 and not self.all_promotion_started:
            return 1.0
    ...
    def update_weight(self):
        ...
        if len(incumbent_configs) < 3:  # fix bug in async: incumbent_configs may be empty
            return
