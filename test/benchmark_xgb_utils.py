...
def mf_objective_func_stopping(config, n_resource, extra_conf, reporter: Reporter,
                               total_resource, x_train, x_val, x_test, y_train, y_val, y_test,
                               n_jobs, run_test=True):
    print('objective extra conf:', extra_conf)
    params = config.get_dictionary()
...
def evaluate_parallel(algo_class, algo_kwargs, method_id, n_workers, dataset, seed, ip, port,
    ...
    if pre_sample:
        ...
        if stopping_variant:
            objective_function = partial(mf_objective_func_stopping, total_resource=R,
        ...
        if stopping_variant:
            ...
            for _ in range(10):
                try:
                    ...
                    worker = async_mqmfWorker_stopping(objective_function, ip, port,
