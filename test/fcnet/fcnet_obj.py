...
from test.resnet.resnet_util import get_path_by_config
...
def get_fcnet_configspace():
    ...
    learning_rate = UniformFloatHyperparameter("learning_rate", 1e-4, 1e-2, default_value=1e-3)
    ...
    batch_size = UniformIntegerHyperparameter("batch_size", 32, 128, default_value=64)
...
def mf_objective_func_gpu(config, n_resource, extra_conf, device, total_resource, run_test=True,
    ...
    try:
        train_loader = DataLoader(dataset=image_data.train_dataset, batch_size=config['batch_size'],
                                  sampler=image_data.train_sampler, num_workers=4)
        ...
        if load_path:
            ...
            cur_epoch_num = checkpoint['epoch_num']
        else:
            cur_epoch_num = 0

        for epoch in range(int(cur_epoch_num), int(cur_epoch_num) + int(epoch_num)):
            model.train()
        ...
        epoch_num_ = int(epoch_num) + int(cur_epoch_num)
