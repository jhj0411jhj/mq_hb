...
python test/benchmark_xgb_async_mfes.py --datasets spambase --R 27 --n_jobs 4 --n_workers 1 --rand_prob 0.3 \
...
def evaluate_parallel(method_id, n_workers, dataset, seed, ip, port):
    ...
    def master_run(return_list):
        async_mfes = async_mqMFES(None, cs, R, eta=eta,
        ...
        async_mfes.run()
        async_mfes.logger.info('===== bracket status: %s' % async_mfes.get_bracket_status(async_mfes.bracket))
        return_list.extend(async_mfes.recorder)  # send to return list
...
for dataset in test_datasets:
    ...
    for i in range(start_id, start_id + rep):
        ...
        method_str = 'amfes-n%d' % (n_workers,)
