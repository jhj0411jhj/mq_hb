...
python test/test_async_hb.py --R 27 --eta 3 --skip_outer_loop 0 --n_workers 1 --runtime_limit 60
...
def evaluate_parallel(method_id, n_workers, seed, ip, port):
    ...
    def master_run(return_list):
        async_hb = async_mqHyperband(None, cs, R, eta=eta,
        ...
        async_hb.run()
        print('===== incumbent =====')
        for config, perf in zip(async_hb.incumbent_configs, async_hb.incumbent_perfs):
            print(config, perf)
        return_list.extend(async_hb.recorder)  # send to return list
...
method_id = 'test_a_hb-n%d-%04d' % (n_workers, seed)
